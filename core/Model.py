...
class Models(object):
      ...
      def LR(self,cv_scoring='roc_auc',cv=3):
          estimator=LogisticRegression(penalty='l2',class_weight=self.class_weight,n_jobs=-1)
          param_grid={'C':np.linspace(0.1,1,100)}
          grid_search = GridSearchCV(estimator=estimator,param_grid=param_grid,scoring=cv_scoring,cv=cv,n_jobs=-1)
          grid_search.fit(self.data,self.target)
          best_parameters = grid_search.best_estimator_.get_params()
          print 'the best estimator is:\n'
          print  grid_search.best_estimator_
          print  best_parameters
          lr_model = LogisticRegression(penalty='l2',class_weight=self.class_weight,n_jobs=-1,C=best_parameters['C'])
